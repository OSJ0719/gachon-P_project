...
export default function ChatbotScreen({ navigation }) {
  ...
  const handleSend = async (text) => {
        ...
        try {
          // 2. 백엔드 호출
          const res = await sendChatbotMessageAPI(userMsg);
          console.log('📦 chatbot response:', res);

          // res 구조: { success, data: { answer, keyPoints, nextActions }, message }
          if (!res || res.success === false || !res.data) {
            // 공통 에러 처리
            const fallback = {
              id: (Date.now() + 1).toString(),
              text: res?.message || '지금은 상담이 어렵습니다.\n잠시 후 다시 시도해 주세요.',
              sender: 'bot',
              type: 'text',
            };
